...
public class StatisticalReportController {
    ...
    @ApiOperation(value="根据计划id-id，计划分类-zbart，获取【计划报表】", notes = "参数：id、zbart")
    @RequestMapping("/getPlanReport")
    public String getPlanReport(@RequestBody ReportVO reportVO) throws Exception{
        List<Map<String, String>> list = statisticalReportService.getPlanReport(
                reportVO.getId(),
                reportVO.getZbart());
        list = reportVO.filter(list);
        Page page = reportVO.initPageImpl(list);
