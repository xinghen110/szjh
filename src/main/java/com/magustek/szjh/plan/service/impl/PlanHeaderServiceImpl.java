    ...
    public List<Map<String, Object>> getCavalByPlanHeadIdAndCaartAndDmart(Long planHeadId, String caart, String dmart, String zbart) {
        ...
        orgMap.keySet().forEach(k->{
            ...
            List<RollPlanHeadDataArchive> rollList = headList.stream()
                    .filter(h -> !Strings.isNullOrEmpty(h.getDtval()) && !Objects.equals("0",h.getDtval()))
                    .collect(Collectors.toList());
...
/*             //合同数量
    ...
    @Override
    public void updateCavalByPlanHeadIdAndZbartAndWears(Long planHeadId, String zbart, BigDecimal wears, String dmart, String dmval) throws Exception {
        log.error("开始调整计算！planId：{}，zbart：{}，wears:{}",planHeadId, zbart, wears);
        Map<Long, List<RollPlanItemDataArchive>> itemMap;
